...
function buildSeed() {
  const cinemas = [
    { id:1, name:'IMAX Октябрь',            address:'Новый Арбат, 24',           city:'Москва', metro:'Арбатская',       rating:4.8, description:'Один из лучших IMAX-залов России. Экран 18×24 м, Dolby Atmos.' },
    { id:2, name:'Синема Парк Vegas',        address:'Каширское шоссе, 61',       city:'Москва', metro:'Домодедовская',    rating:4.6, description:'Мегаплекс в ТЦ Vegas. 14 залов, IMAX и 4DX.' },
    { id:3, name:'Формула Кино Европейский', address:'Киевская пл., 2',           city:'Москва', metro:'Киевская',         rating:4.5, description:'В центре Москвы, три линии метро рядом.' },
    { id:4, name:'Люмьер',                   address:'ул. Пятницкая, 25с1',      city:'Москва', metro:'Третьяковская',    rating:4.7, description:'Артхаусный кинотеатр, авторское кино и ретроспективы.' },
    { id:5, name:'Каро 11 Октябрьское поле', address:'ул. Маршала Бирюзова, 32', city:'Москва', metro:'Октябрьское поле', rating:4.4, description:'11 залов, Dolby Cinema, на севере Москвы.' },
  ];
  ...
  const sessions = [];
  const seats = [];
  let hallId = 1, sessionId = 1, seatId = 1;
...
function getDB() {
  let db = readDB();
  if (!db) { db = buildSeed(); writeDB(db); console.log('✅ База создана'); }
  return db;
}
...
app.get('/api/cinemas', (req, res) => {
  ...
  const result = db.cinemas.map(c => ({
    ...c,
    session_count: db.sessions.filter(s => s.cinema_id === c.id && s.date >= today).length,
  }));
...
app.get('/api/cinemas/:id/sessions', (req, res) => {
  ...
  list.sort((a,b) => a.date.localeCompare(b.date)||a.time_start.localeCompare(b.time_start));
...
app.get('/api/sessions/:id/seats', (req, res) => {
  ...
  const seats = db.seats.filter(s => s.session_id === sid)
...
app.post('/api/orders', (req, res) => {
  ...
  const selectedSeats = db.seats.filter(s => seat_ids.includes(s.id) && s.session_id === +session_id);
  ...
  if (taken.length)
    return res.status(409).json({ ok:false, error:'Seats already taken', taken: taken.map(s=>s.id) });

  const priceMap = { standard: session.price_standard, comfort: session.price_comfort, vip: session.price_vip };
  const total    = selectedSeats.reduce((sum,s)=>sum+(priceMap[s.category]||session.price_standard),0);

  const orderId = nextId(db.orders);
  const order   = { id:orderId, session_id:+session_id, user_token, status:'pending', total_price:total, created_at: new Date().toISOString(), paid_at:null, cancelled_at:null };
  db.orders.push(order);

  const newTickets = selectedSeats.map(seat => {
    const price   = priceMap[seat.category]||session.price_standard;
    const ticket  = { id:nextId(db.tickets), order_id:orderId, session_id:+session_id, seat_id:seat.id, category:seat.category, price, barcode:`CT-${orderId}-${seat.id}`, row_num:seat.row_num, seat_num:seat.seat_num, created_at:new Date().toISOString() };
...
app.put('/api/orders/:id/pay', (req, res) => {
  ...
  if (order.status !== 'pending') return res.status(400).json({ ok:false, error:`Cannot pay: ${order.status}` });
...
app.put('/api/orders/:id/cancel', (req, res) => {
  ...
  const order = db.orders.find(o => o.id === +req.params.id);
  if (!order)                   return res.status(404).json({ ok:false, error:'Not found' });
  if (order.status==='cancelled') return res.status(400).json({ ok:false, error:'Already cancelled' });
