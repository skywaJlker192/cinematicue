...
function spinWheel(canvas, onDone) {
  ...
  const angleDelta =
    ...
    Math.ceil((targetAngle - startAngle) / (2 * Math.PI)) * 2 * Math.PI;
...
function applyDiscountToPrices(discount) {
  const plans = [
    {
      el: document.querySelector('.plan-price-basic'),
      base: BASE_PRICES.basic
    },
    {
      el: document.querySelector('.plan-price-standard'),
      base: BASE_PRICES.standard
    },
    {
      el: document.querySelector('.plan-price-premium'),
  ...
  plans.forEach(({ el, base }) => {
    ...
    if (badge) {
      ...
      badge.textContent =
        discount >= 1 ? 'FREE' : `-${Math.round(discount * 100)}%`;
    }
    current.innerHTML =
      discount >= 1
...
function handlePromoApply() {
  ...
  if (found && found.discount > 0) {
    ...
    if (status) {
      ...
      status.innerHTML =
        '<i class="fas fa-times-circle"></i> Промокод не найден или недействителен';
      setTimeout(() => {
        if (status) status.textContent = '';
      }, 3000);
    }
...
function showWinResult(prize) {
  ...
  document
    .getElementById('btn-activate-promo')
    ?.addEventListener('click', () => {
      activatePromo(prize);
      showWheelToast(
        `${prize.label} активирована! Промокод ${prize.code}`,
        'prize'
      );
      // скролл к ценам
      document
...
function getLastSpin() {
  return +localStorage.getItem('cine_wheel_ts') || 0;
}
function setLastSpin() {
  localStorage.setItem('cine_wheel_ts', Date.now());
}
function canSpin() {
  return Date.now() - getLastSpin() > 24 * 60 * 60 * 1000;
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  modal?.addEventListener('click', (e) => {
    if (e.target === modal) closeWheelModal();
  });
  ...
  spinBtn?.addEventListener('click', () => {
    ...
    spinWheel(canvas, (prize) => {
      ...
      showWinResult(prize);
      showWheelToast(
        prize.code ? `Выпало: ${prize.label}!` : 'Не повезло в этот раз :(',
        prize.code ? 'prize' : 'info'
      );
  ...
  document
    ...
    ?.addEventListener('click', handlePromoApply);
  ...
  window.__wheelReset = () => {
    ...
    updateSpinBtn(spinBtn);
    console.log(
      '%c✅ Колесо сброшено! Можно крутить.',
      'color:#cca43b;font-size:14px;font-weight:bold;'
    );
  };
