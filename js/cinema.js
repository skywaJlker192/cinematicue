...
async function updateSubscription(status) {
  ...
  try {
    ...
    showToast(
      status ? 'Подписка активирована' : 'Подписка отключена',
      'success'
    );
    updateUIForAuth();
  } catch (e) {
...
function goToStep(n) {
  currentStep = n;
  document
    .querySelectorAll('.booking-step-content')
...
function renderCinemas(list) {
  const grid = document.getElementById('cinemas-grid');
  if (!list.length) {
    grid.innerHTML = '<p class="no-results">Кинотеатры не найдены</p>';
    return;
  }
  grid.innerHTML = list
    .map(
...
async function selectCinema(id) {
  ...
  document.getElementById('sessions-cinema-name').textContent =
    state.selectedCinema.name;
  document.getElementById('sessions-cinema-addr').textContent =
    `${state.selectedCinema.address} · м. ${state.selectedCinema.metro || ''}`;
  goToStep(2);
...
async function loadSessions(cinemaId) {
  document.getElementById('sessions-list').innerHTML =
    '<div class="sessions-loading"><i class="fas fa-spinner fa-spin"></i> Загружаем расписание...</div>';
  try {
  ...
  if (
    ...
    document.getElementById('sessions-list').innerHTML =
      '<p class="no-results">В этом кинотеатре пока нет сеансов</p>';
    return;
  }
  ...
  try {
    ...
    document.getElementById('sessions-list').innerHTML =
      '<p class="no-results">Не удалось загрузить расписание</p>';
  }
}
...
function renderSessionsForDate(date) {
  ...
  if (!filtered.length) {
    list.innerHTML = '<p class="no-results">На эту дату нет сеансов</p>';
    return;
  }

  const byFilm = {};
  filtered.forEach((s) => {
    (byFilm[s.film_title] = byFilm[s.film_title] || []).push(s);
  });

  list.innerHTML = Object.entries(byFilm)
    .map(
      ([film, sessions]) => `
...
async function selectSession(id) {
  ...
  const dateLabel = new Date(s.date + 'T00:00:00').toLocaleDateString('ru', {
    day: 'numeric',
    month: 'long'
  });
  document.getElementById('seats-session-info').textContent =
    `${s.film_title} · ${dateLabel} · ${s.time_start} · ${s.format}`;
  document.getElementById('pib-vip-price').textContent = `₽${s.price_vip}`;
  document.getElementById('pib-comfort-price').textContent =
    `₽${s.price_comfort}`;
  document.getElementById('pib-standard-price').textContent =
    `₽${s.price_standard}`;
...
async function loadSeats(sessionId) {
  ...
  map.innerHTML =
    '<div class="seats-loading"><i class="fas fa-spinner fa-spin"></i></div>';
  try {
    if (state.offline) throw new Error('offline');
...
function renderSeatMap() {
  ...
  for (let r = 1; r <= rows; r++) {
    ...
    for (let c = 1; c <= cols; c++) {
      const seat = state.allSeats.find(
        (s) => s.row_num === r && s.seat_num === c
      );
      if (!seat) {
      ...
      const price =
        {
          vip: s.price_vip,
          comfort: s.price_comfort,
          standard: s.price_standard
        }[seat.category] || s.price_standard;
...
function toggleSeat(el) {
  ...
  if (state.selectedSeatIds.has(id)) {
    ...
    if (state.selectedSeatIds.size >= 6) {
      showToast('Максимум 6 мест за раз', 'info');
      return;
    }
...
function renderSelectionSummary() {
  ...
  document.getElementById('sel-count').textContent =
    `${count} ${count === 1 ? 'место' : count < 5 ? 'места' : 'мест'} выбрано`;
  document.getElementById('sel-seats-list').textContent = seatLabels.join(', ');
  document.getElementById('sel-total-price').textContent =
...
function proceedToPayment() {
  if (!state.selectedSeatIds.size) {
    showToast('Выберите хотя бы одно место', 'info');
    return;
  }
  const s = state.selectedSession;
  ...
  document.getElementById('pay-final-total').textContent =
    `₽${total.toLocaleString('ru')}`;
...
async function processPayment() {
  ...
  try {
    ...
    if (state.offline) {
      order = {
        ...
        total_price: [...state.selectedSeatIds].reduce((sum, id) => {
          ...
          if (!seat) return sum;
          const p = {
            vip: state.selectedSession.price_vip,
            comfort: state.selectedSession.price_comfort,
            standard: state.selectedSession.price_standard
          };
      ...
      const paid = await apiFetch(`${API}/orders/${created.order.id}/pay`, {
        method: 'PUT'
      });
      order = paid.order;
    ...
    btn.disabled = false;
    btn.innerHTML =
      '<i class="fas fa-lock"></i> Оплатить <span id="pay-btn-amount">' +
      document.getElementById('pay-btn-amount')?.textContent +
      '</span>';
  }
}
...
function generateBarcodeSVG(code) {
  const bars = Array.from(
    { length: 30 },
    (_, i) =>
      `<rect x="${i * 3}" y="0" width="${1 + (i % 3)}" height="30" fill="currentColor" opacity="${0.5 + Math.random() * 0.5}"/>`
  ).join('');
...
function updateOrdersBadge() {
  ...
  const saved = state.offline
    ? JSON.parse(localStorage.getItem('cine_orders') || '[]')
    : [];
...
async function openOrdersModal() {
  ...
  list.innerHTML =
    '<div class="orders-loading"><i class="fas fa-spinner fa-spin"></i> Загружаем...</div>';
  ...
  if (!orders.length) {
    list.innerHTML =
      '<div class="no-orders"><i class="fas fa-ticket-alt"></i><p>Заказов пока нет</p></div>';
    return;
  }
  ...
  list.innerHTML = orders
    .map((order) => {
      const dateLabel = order.date
        ? new Date(order.date + 'T00:00:00').toLocaleDateString('ru', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          })
      ...
      const statusText =
        { paid: 'ОПЛАЧЕН', pending: 'ОЖИДАЕТ ОПЛАТЫ', cancelled: 'ОТМЕНЁН' }[
          order.status
        ] || order.status;
      return `
...
async function cancelOrder(id) {
  try {
    if (state.offline) {
      ...
      if (idx >= 0) {
        saved[idx].order.status = 'cancelled';
        localStorage.setItem('cine_orders', JSON.stringify(saved));
      }
    } else {
  ...
  document.querySelectorAll('.cf-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      const filtered =
        metro === 'all'
          ? state.cinemas
          : state.cinemas.filter((c) => c.metro === metro);
  ...
  window.addEventListener('load', () => {
    ...
    loadSessions = async function (cinemaId) {
      ...
      if (dates.length) {
        ...
        document.getElementById('sessions-list').innerHTML =
          '<p class="no-results">Нет сеансов этого фильма в выбранном кинотеатре</p>';
      }
    };
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Theme
  const toggle =
    document.getElementById('theme-toggle-cinema') ||
    document.querySelector('.bb8-toggle__checkbox');
  if (toggle) {
  ...
  if (document.getElementById('cinemas-grid')) {
    ...
    document.querySelectorAll('.pay-method').forEach((el) => {
      el.querySelector('input')?.addEventListener('change', () => {
        ...
        if (cardVisual)
          cardVisual.style.display = val === 'card' ? 'flex' : 'none';
      });
    });
    ...
    document
      .getElementById('btn-download-tickets')
      ?.addEventListener('click', () => {
    ...
    document
      .getElementById('my-orders-fab')
      ?.addEventListener('click', openOrdersModal);
    document
      .getElementById('orders-modal-close')
      ?.addEventListener('click', () =>
  ...
  const topBtn =
    ...
    document.getElementById('back-to-top-index');
  if (topBtn) {
